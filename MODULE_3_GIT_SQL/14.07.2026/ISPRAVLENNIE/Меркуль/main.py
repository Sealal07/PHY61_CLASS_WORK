...
async def seed_data():
    ...
    async with async_session_maker() as session:
        ...
        inst1 = Instructor(name='Алексей Петров', email='a@school.com')
        ...
        inst2.profile = InstructorProfile(bio='Data Science ', experience_years=5)
# ДОБАВЛЕН ОТСТУП
        # 2. Курсы
        course1 = Course(title='Python с нуля до профи', price=15000, instructor=inst1)
        course2 = Course(title='Асинхронное программирование на Python', price=25000, instructor=inst1)
...
async def run_queries():
    ...
    # joinedload - для 1 : 1 (left join)
    # selectinload - для 1 : М или М : М

    print('2.Курсы с подгрузкой связей')
    query = (select(Course).options(joinedload(Course.instructor), selectinload(Course.students)))
    result = await session.execute(query)
    ...
    for course in courses:
        ...
        print(f'{course.title} | {inst_name} | {student_names}')


    # 3. Фильтрация и сортировка
        print('3. Фильтрация курсов по цене и сортировка по убыванию')
        query = (select(Course).where(Course.price > 15000).order_by(Course.price.desc()))
        result = await session.execute(query)
        courses = result.scalars().all()
        for c in courses:
        ...
        query = select(Instructor).where(or_(Instructor.name.like("А%"), Instructor.email.like("%.com")))
        result = await session.execute(query)
