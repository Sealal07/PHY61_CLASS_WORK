...
class InstructorProfile(Base):
    ...
    experience_years: Mapped[int] = mapped_column(default=0)  # Исправлено: experience_years (было expertience_years)
...
class Instructor(Base):
    ...
    email: Mapped[str] = mapped_column(String(150), unique=True, index=True)  # Исправлено: String (с большой буквы)
...
class Course(Base):  # Исправлено: Course (было course) - теперь с большой буквы
...
class Student(Base):
    ...
    courses: Mapped[List["Course"]] = relationship(  # Исправлено: courses (было course)   # Исправлено была фигурная скобка закрывающая
        secondary=student_course_association,
