...
async def seed_data():
    ...
    async with async_session_maker() as session:
        ...
        course2 = Course(title='Асинхронное программирование на Python', price=30000, instructor=inst1)
        course3 = Course(title='Введение в Data Scientist', price=3000, instructor=inst2)
        ...
        await session.commit() # выполняется INSERT
...
async def run_queries():
    ...
    async  with async_session_maker() as session:
        ...
        # .execute()
        query = select(Instructor)  # Добавила эту строку, она отсутствует
        ...
        for course in courses:
            ...
            # Опечатка: inst вместо inst_name
            print(f'{course.title} | {inst.name} | [{student_names}]')  # Нужно использовать inst_name или course.instructor.name
            ...
            query = (
                ...
                # Опечатка: desp вместо desc
                .order_by(Course.price.desc())  # Было desp
            )
            result = await session.execute(query)
            courses = result.scalars().all()
            for c in courses:
