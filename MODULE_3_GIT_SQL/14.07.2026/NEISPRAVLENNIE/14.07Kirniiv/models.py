...
# Mapped[int] - указывает Python что поле при извлечении из БД
...
class InstructorProfile(Base):
    ...
    #  Обратная связь "1:1"
    instructor: Mapped["Instructor"] = relationship(backref='profile')
...
class Course(Base):
    ...
    price: Mapped[str] = mapped_column(String(150), unique=True, index=True)
    instructor_id:Mapped[int] =  mapped_column(ForeignKey("instructors.id", ondelete="SET NULL"), unique=True)
